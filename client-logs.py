    ...
    while True:
        while True:
            ...
            if reconect_time is not None:
                result = subprocess.run(['docker', 'logs', '-t', '--since', reconect_time, container_name], stdout=subprocess.PIPE)
            ...
            if not ready:
                print('Since')
                result = subprocess.run(['docker', 'logs', '-t', '--since', '299s', container_name], stdout=subprocess.PIPE)
                yield result.stdout.decode('utf-8')
                break
...
if __name__ == "__main__":
    container_name = "streamlive-3413"
    db_name = "Test"
    host = "127.0.0.1"
    port = 34092
